...
class UserWallet(models.Model):
    ...
    identity = models.CharField(max_length=100, unique=True, null=True, blank=True)
...
class PatientRecordBlockchain(models.Model):
    """Track patient records registered in blockchain"""
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE, related_name='blockchain_records')
    doctor = models.ForeignKey(Doctor, on_delete=models.CASCADE)
    transaction_id = models.CharField(max_length=500, unique=True)  # Increased length
    record_hash = models.TextField()
    block_hash = models.CharField(max_length=500, blank=True, null=True)  # Increased length
    ...
    class Meta:
        ...
        verbose_name = 'Patient Record Blockchain Entry'
        verbose_name_plural = 'Patient Records Blockchain Entries'
    
    def __str__(self):
        return f"Blockchain record for {self.patient.user.get_full_name()} - {self.registration_date}"
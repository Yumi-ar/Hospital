...
if __name__ == "__main__":
    ...
    if client.test_connection():
        ...
        hash_id = client.create_patient_record(patient_data)
        if hash_id:
            print(f"Patient record stored with hash: {hash_id}")
            
            # Pin the record for persistence
            client.pin_file(hash_id)
            
            # Retrieve and verify
            retrieved_data = client.get_json_data(hash_id)
            if retrieved_data:
                print("Retrieved patient data:")
                print(json.dumps(retrieved_data, indent=2))
    else:
        print("Please make sure IPFS daemon is running: ipfs daemon")